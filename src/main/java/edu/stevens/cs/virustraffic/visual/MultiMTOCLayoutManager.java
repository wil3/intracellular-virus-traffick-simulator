 ...
 *
 */
public class MultiMTOCLayoutManager implements CellLayoutManager {
	
	private JGraphXAdapter<MicrotubleVertex, Microtuble> graph;
	private Random rand = new Random();
	private CellMorphology morphology;
	
	public MultiMTOCLayoutManager( CellMorphology morphology){
	...
	public void createLayout(double x_origin, double y_origin){
		...
		for (mxCell sudo_mtoc : getCellByType(VertexType.SUDO_MTOC)){
			
			int angle = rand.nextInt(360);
			double distance = 100;
			double y = Math.sin(angle) * (double)distance;
			double x = Math.cos(angle) * (double)distance;
			MicrotubleVertex v = (MicrotubleVertex)sudo_mtoc.getValue();
			graph.getModel().setStyle(sudo_mtoc, v.getStyle());
			graph.getModel().setGeometry(sudo_mtoc, new mxGeometry(x + x_origin, y+ y_origin, v.getSize().getWidth(), v.getSize().getHeight()));
		}
		...
		//handle the rest
		addReceptors(x_origin, y_origin);
	 ...
	 * @param y
	 */
	private void addReceptors(double x, double y){
